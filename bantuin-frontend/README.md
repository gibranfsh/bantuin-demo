...
> A local peer-to-peer helper marketplace for the Unpad Jatinangor campus community.

Bantu.in helps students ask for everyday favors — food runs, deliveries, tutoring, moving help — and connect with nearby classmates who can help. It is inspired by the **Nangor to Help** community and the menfess workflows (ITBFess, UnpadFess) that students already use, but replaces scattered WhatsApp threads with structured task posts, status tracking, and in-app coordination.
...
Bantu.in is a campus task marketplace built for students at **Universitas Padjadjaran, Jatinangor**. Instead of posting a need on menfess and negotiating everything over private WhatsApp chats, students can create a structured request, track its progress, and coordinate with a helper in one place.

Typical use cases include:

- Titip beli makanan or barang (food runs)
- Package delivery around campus
- Tutoring or thesis formatting help
- Moving or errand tasks between dorm blocks and faculty buildings

The product is offered **free of charge** — there are no platform fees and no monetization in this MVP. The goal is to validate whether a dedicated platform improves on the existing manual workflow before investing in backend infrastructure.

---

## The problem

Today, students in Jatinangor often find or offer help through **Nangor to Help**, **ITBFess**, **UnpadFess**, and similar channels. The flow works, but it creates friction:
...
- Coordination history is scattered across multiple WhatsApp threads
...
Rather than launching a full marketplace and hoping for adoption, Bantu.in starts with a **partner-first, validation-first** mindset:

- Work with existing communities like **Nangor to Help** that already have active demand
...
- Keep WhatsApp as a viable channel if users prefer it — the platform should complement, not fight, existing habits
- Ship a demo-quality frontend first to learn whether structured requests feel better than menfess + chat

This repository contains that frontend MVP: a fully interactive demo with mock data, designed for walkthroughs and user feedback.
...
|------|--------------|--------------|
| **Helpee** | Student who needs help | Posts a task, agrees on price, verifies work, releases payment |
| **Helper** | Student who offers help | Browses open tasks, offers to help, executes the favor, submits completion notes |
...
| Tab | Features |
|-----|----------|
| **Browse (Home)** | Hero with open-task count; category chips (All, Food run, Tutoring, Moving); status tabs (Open, Claimed) with dynamic counts; navbar search across title, description, category, and location; `?mine=1` filter for the user's posted tasks; responsive request-card grid; role-specific empty states; loading skeletons |
...
| **Directory** | Community helper cards with star rating, tasks completed, on-time percentage, and verified-student badge |
...
- **Interactive Leaflet map** with Jatinangor landmarks (FEB Canteen, Central Library, Jatinangor Dorm, Rektorat, Main Gate) and map pinpoint selection
...
- User avatar and verified-student badge
...
- Seed data uses Jatinangor-localized tasks ([`src/lib/mock-data.ts`](src/lib/mock-data.ts))
...
- **Maps:** Leaflet (Jatinangor landmark picker on the post form)
...
Try the full task lifecycle in a few minutes:
...
- [`brainstorming-result.md`](brainstorming-result.md) — Product vision, user journeys, and validation questions (Indonesian)
